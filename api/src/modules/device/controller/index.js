...
const getItems = async (request, h) => {
  try {
    ...
    return Object.assign(await Device.getPaginate(query, dataInit), await Device.getCountByStatus(query))
  } catch (error) {
...
const getItem = async (request, h) => {
  try {
    ...
    if (!(params._id && mongoose.Types.ObjectId.isValid(params._id))) {
      return Boom.badRequest('Device Id not valid.')
    }
    let item = await Device.findById(params._id)
    if (!item) {
...
const save = async (request, h) => {
  try {
    ...
    if (payload._id) {
      ...
      item = await Device.findById(payload._id)
      if (!item) {
        return Boom.notFound('Device not found.')
      }
      ...
      item = await Device.create(payload)
    }
    return item
  } catch (error) {
    return Boom.badRequest(error)
